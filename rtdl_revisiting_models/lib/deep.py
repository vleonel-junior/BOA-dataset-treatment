...
class IndexLoader:
    ...
    def __iter__(self):
        # Compatibilité avec différentes versions du package `zero` :
        # - anciennes versions exposaient `zero.iloader`
        # - certaines exposent `zero.data.IndexLoader`
        # On essaie les deux, puis on retombe sur un fallback séquentiel.
        try:
            indices = list(
            ...
            try:
                indices = list(
                    zero.data.IndexLoader(self._train_size, self._batch_size, shuffle=self._shuffle)
                )
            except Exception:
                # Fallback : générer des indices séquentiels
                idx = torch.arange(self._train_size, device=self._device)
                indices = [idx[i : i + self._batch_size] for i in range(0, self._train_size, self._batch_size)]
